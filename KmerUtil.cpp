...
/*kmersUtil::kmersUtil(){}
kmersUtil::~kmersUtil(){}*/
void kmersUtil::add2table(kmercode_length kmer,unsigned int count)
{
  counting_hash_table[kmer]=count;

}
unsigned int kmersUtil::get_from_table (kmercode_length kmer)
{
  if (counting_hash_table.count(kmer)==0)
   return 0;
  else
   return counting_hash_table[kmer]; 

} 
int kmersUtil::nt2int(char nt)
{
...
int kmersUtil::first_nt(kmercode_length kmer)
{
...
int kmersUtil::code2nt(kmercode_length code, int which_nt)
{
...
kmercode_length kmersUtil::get_canonical_kmer_code(kmercode_length code)
{
    int i ;
    kmercode_length rev_com = get_reverse_complement(code);
    return rev_com < code ? rev_com : code ;
}
kmercode_length kmersUtil::get_reverse_complement(kmercode_length code)
{
    int i ;
...
int kmersUtil::code2seq (kmercode_length code, char seq[])
{
    int i;
    kmercode_length temp = code;
...
kmercode_length kmersUtil::next_kmer(kmercode_length kmer_code, int added_nt, int *strand)
{
...
void kmersUtil::char_revcomp(char nt,char &cnt)
{
    if(nt=='A'||nt=='a')
        {cnt='T'; return;}
    if(nt=='C'||nt=='c')
...
void kmersUtil::revcomp_sequence(std::vector<char> &sequence, int len)
{
		  int i;
		  unsigned char t;
		  for (i=0;i<len/2;i++)
...
void kmersUtil::get_kmers_one_read(std::string read, std::vector<kmercode_length> &kmers_list)
{
...
float kmersUtil::needleman_wunch(std::string a, std::string b)
{
    float gap_score = -5;
...
void kmersUtil::get_reads_spiliting_around_Ns(std::string read_seq,int read_length,std::vector<std::string> & reads)
{
          int i=0;
          int indx=0;
